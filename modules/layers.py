...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        if not self.training:
            ...
            self.var = np.mean(self.input_mean ** 2, axis=0, keepdims=True)
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        :return: array of shape (batch_size, num_features)
        """
        grad_input = grad_output.copy()# я забыл сделать .copy() и в итоге не мог раздебажить update_grad_parameters где-то сутки. я ненавижу питон...
        
        if self.affine:
            grad_input *= self.weight
        
        if not self.training:
            return grad_input / np.sqrt(self.running_var + self.eps)

        grad_var = -0.5 * np.sum(grad_input * self.input_mean, axis=0, keepdims=True) * self.inv_sqrt_var ** 3

        grad_mean = -np.sum(grad_input * self.inv_sqrt_var, axis=0, keepdims=True) + grad_var * -2 * np.mean(self.input_mean, axis=0, keepdims=True)
        
        B = input.shape[0]

        return grad_input * self.inv_sqrt_var + grad_var * 2 / B * self.input_mean + grad_mean / B
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        self.mask = np.random.binomial(n=1, p=1-self.p, size=input.shape) / (1 - self.p)
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        for i in range(len(self.modules) - 1, -1, -1):
            ...
            if i == 0:
                ...
                model_in = self.modules[i - 1].output
            
            grad_input = self.modules[i].backward(model_in, grad_out)
